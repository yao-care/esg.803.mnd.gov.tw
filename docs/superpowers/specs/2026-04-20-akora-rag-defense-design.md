...
檔案路徑：`_meta/glossary.json`（領域資料，與 `_meta/rule.md` 同層）
...
- **QA 驗證（qa-report.js）**：在 main loop 中每道題目呼叫 `findRelevantChunks()` 之前，先做 `expandGlossary(q.question, glossary)`。glossary 從 `_meta/glossary.json` 直接讀取（`JSON.parse(readFileSafe(...))`），不依賴 build。
